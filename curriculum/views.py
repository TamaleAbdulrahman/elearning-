...
class LessonDetailView(DetailView, FormView):
    ...
    def get_object(self, queryset=None):
        standard_slug = self.kwargs.get('standard')
        ...
        return Lesson.objects.filter(
            ...
            Standard__slug=standard_slug  # adjust 'Standard' if your model uses a different related name or FK
        ).first()
    ...
    def form2_valid(self, form):
        ...
        fm = form.save(commit=False)
        fm.author = self.request.user
        fm.comment_name_id = self.request.POST.get('comment_id')
        fm.save()
        return HttpResponseRedirect(self.get_success_url())
...
class LessonUpdateView(UpdateView):
    ...
    def get_object(self, queryset=None):
        ...
        ).first()  # Use get() if you're 100% sure this combo is unique
